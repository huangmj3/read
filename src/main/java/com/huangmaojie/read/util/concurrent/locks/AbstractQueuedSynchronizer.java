...
package com.huangmaojie.read.util.concurrent.locks;

import com.huangmaojie.read.util.concurrent.CountDownLatch;
import com.huangmaojie.read.util.concurrent.TimeUnit;
...
public abstract class AbstractQueuedSynchronizer
    ...
    // 抽象的对列式同步器
    // 核心思想式提供了一个同步对列，将未获取到锁的线程阻塞排队
    // 用到了模版方法模式，方法的具体实现由子类进行提供
    ...
    static final class Node {
         ...
         */
        volatile int waitStatus;   // 等待状态，默认为0
    ...
    protected boolean tryAcquire(int arg) {
        // 抛出的这个异常是一个默认实现独占的时候需要子类去实现。而共享的时候则可以不用理会。
     ...
     */
    // 通过tryAcquire尝试获取独占锁，如果成功返回true,失败放回false。
