...
import com.huangmaojie.read.util.concurrent.atomic.AtomicInteger;
import com.huangmaojie.read.util.concurrent.locks.AbstractQueuedSynchronizer;
import com.huangmaojie.read.util.concurrent.locks.Condition;
...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
            ...
            // 2.(第二个判断)SHUTDOWN状态接收新任务，但是仍然会执行已经加入任务队列的任务，所以当进入SHUTDOWN状态，
            // 而传进来的任务为空，并且任务队列不为空的时候，是允许添加新线程的，如果把这个条件取反，就表示不允许添加worker。
            if (rs >= SHUTDOWN &&
                ! (rs == SHUTDOWN &&
                   firstTask == null &&
                   ! workQueue.isEmpty()))
     ...
     */
    /**
     * 如果worker已经构造好了，则从workers集合中移除这个woker
    ...
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            try {
                // 根据 timed 来判断，如果为true，则通过阻塞队列 poll 方法进行超时控制，如果在 keepaliveTime时间内没有获取到任务，
                // 则返回null。否则通过take方法阻塞式获取队列中的任务
                Runnable r = timed ?
    ...
    /**
     * 如果task不为空，则开始执行task
     * 如果task为空，则通过getTask()再去取任务，并复制给task，如果取到的Runnable不为空，则执行该任务
     * 执行完毕后，通过while循环继续getTask()取任务。
     * 如果getTask()渠道的任务依然是空，那么整个runWorker()方法执行完毕。
     */
    final void runWorker(Worker w) {
        Thread wt = Thread.currentThread();
        Runnable task = w.firstTask;
        w.firstTask = null;
        // unlock，表示当前worker线程允许中断，因为new Worker默认的state=-1，此处是调用Worker类的tryRelease()方法，将state置为0，
