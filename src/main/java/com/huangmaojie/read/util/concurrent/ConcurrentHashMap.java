...
package com.huangmaojie.read.util.concurrent;

import com.huangmaojie.read.util.concurrent.atomic.AtomicReference;
import com.huangmaojie.read.util.concurrent.atomic.LongAdder;
import com.huangmaojie.read.util.concurrent.locks.LockSupport;
import com.huangmaojie.read.util.concurrent.locks.ReentrantLock;
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K, V>[] tab = table; ; ) {
            ...
            if (tab == null || (n = tab.length) == 0)
                ...
                // 如果待插入的table数组下标为空，则直接利用cas插入一个node
                if (casTabAt(tab, i, null,
                        new Node<K, V>(hash, key, value, null)))
                    break;                   // no lock when adding to empty bin
            } else if ((fh = f.hash) == MOVED)
                // 如果table数组正在扩容，则帮助该线程扩容
                tab = helpTransfer(tab, f);
            else {
                // 记录要被替换的老数据
                V oldVal = null;
                // 对table数组对应下标位置的根节点枷锁
                synchronized (f) {
                    if (tabAt(tab, i) == f) {
        ...
        // 增加计数数组或baseCount的数量，baseCount+轮训后的counterCells才是最终的元素个数
    ...
    private final Node<K, V>[] initTable() {
        // sizeCtl标志是在Node数组初始化或者扩容的时候一个控制位标识，负数代表正在进行初始化或者扩容操作。
        // -1代表正在初始化
            ...
            if ((sc = sizeCtl) < 0)
                ...
                try {
                    if ((tab = table) == null || tab.length == 0) {
                        ...
                        // 将新数组赋值给table
                        table = tab = nt;
                        // sc,计算容器容量多少的时候触发扩容，实际就是当前容量的0.75倍，这里使用了右移来计算
                        sc = n - (n >>> 2);
                    }
                } finally {
     ...
     * 它把Node数组当做多个线程之间共享的任务队列，
     * 然后通过维护一个指针来划分每个线程锁负责的区间，
     * 每个线程通过区间逆向遍历来实现扩容，
     * 一个已经迁移完的bucket会被替换成一个ForwardingNode节点，标记当前bucket已经被其他线程迁移完了
     * <p>
     * <p>
     * fwd：这个类是表示类，用于指向新表用的，其他线程遇到这个类会主动跳过这个类，因为这个类要么就是扩容迁移正在进行，要么就是已经完成扩容迁移，也就是这个类要保证线程安全，在进行操作。
    ...
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        ...
        // 创建一个fwd节点，表示一个正在被迁移的Node，并且它的hash值为-1（MOVED），
        ...
        // 通过自循环处理每个槽位中的链表元素，默认advace为真，通过CAS设置transferIndex属性值，并初始化i和bound值，
        ...
        for (int i = 0, bound = 0; ; ) {
            ...
            if (i < 0 || i >= n || i + n >= nextn) {
                ...
                // 这里使用CAS操作对sizeCtl的低16位进行减1，代表做完了属于自己的任务
                if (U.compareAndSwapInt(this, SIZECTL, sc = sizeCtl, sc - 1)) {
                    // 第一个扩容的线程，执行transfer方法之前，会设置sizeCtl=(resizeStamp(n)<<RESIZE_STAMP_SHIFT)+2,
                    // 后续帮其扩容的线程，执行transfer方法之前，会这只sizeCtl=sizeCtl+1每一个退出transfer的方法的线程，
                    // 退出之前，会这只sizeCtl=sizeCtl-1那么最后一个线程退出时，必然有sc == (resizeStamp(n) << RESIZE_STAMP_SHIFT) + 2)，
                ...
                synchronized (f) {
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            // 在旧的hash桶中放置转发节点（fwd），表明此hash桶已经处理
                            setTabAt(tab, i, fwd);
                            advance = true;
                        } else if (f instanceof TreeBin) {
                            TreeBin<K, V> t = (TreeBin<K, V>) f;
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                ...
                if ((a = as[(n - 1) & h]) == null) {
                    if (cellsBusy == 0) {            // Try to attach new Cell
                        ...
                        if (cellsBusy == 0 &&
                                // cellsBusy 表示当前cell数组是否在初始化或扩容中的CAS标志位
